...
class DynamicArray:
    ...
    def resize(self, new_capacity: int) -> None:
        ...
        if new_capacity <= 0 or new_capacity < self.size:
            ...
            self.data = StaticArray(new_capacity)
            self.capacity = new_capacity

            # I don't like this but seems to be the way to copy old elements after resizing
            for i in range(self.size):
                self.data[i] = temp_array[i]
    ...
    def insert_at_index(self, index: int, value: object) -> None:
        ...
        if index < 0 or index > self.size:
            ...
            temp = [None] * self.size
            for i in range(self.size):
                temp[i] = self.get_at_index(i)

            self.size += 1
            self.set_at_index(index, value)
            for i in range(index + 1, self.size):
                self.set_at_index(i, temp[i - 1])
    ...
    def remove_at_index(self, index: int) -> None:
        ...
        for i in range(index, self.size):
            self.set_at_index(i, temp_arr[i + 1])
    ...
    def slice(self, start_index: int, size: int) -> object:
        ...
        """
        newArr = DynamicArray()
        if 0 <= start_index < self.size and start_index + size <= self.size and size >= 0:
            for i in range(start_index, start_index + size):
                newArr.append(self.get_at_index(i))
        else:
            raise DynamicArrayException
    ...
    def map(self, map_func) -> object:
        ...
        for i in range(self.size):
            newDynArr.append(map_func(self.get_at_index(i)))
        return newDynArr
    ...
    def filter(self, filter_func) -> object:
        ...
        for i in range(self.size):
            if filter_func(self.get_at_index(i)):
                newDynArr.append(self.get_at_index(i))
        return newDynArr
...
def find_mode(arr: DynamicArray) -> (DynamicArray, int):
    ...
    for i in range(arr.size):
        if arr.get_at_index(i) not in dict:
            dict[arr.get_at_index(i)] = 1
        else:
            dict[arr.get_at_index(i)] += 1

    max_vals = max(dict.values())

    for key in dict:
        if dict[key] == max_vals:
            newDynArr.append(key)

    return newDynArr, max_vals
