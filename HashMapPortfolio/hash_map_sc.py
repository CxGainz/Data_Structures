...
class HashMap:
    ...
    def get(self, key: str) -> object:
        ...
        cur = self.buckets.get_at_index(index).contains(key)
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        for i in range(self.buckets.length()):
            if self.buckets.get_at_index(i).length() != 0:
                ...
                for cur in self.buckets.get_at_index(i):

                    index = newHM.hash_function(cur.key) % newHM.capacity

                    if newHM.buckets.get_at_index(index).contains(cur.key) is not None:
                        newHM.buckets.get_at_index(index).remove(cur.key)
                        newHM.size -= 1
                    newHM.buckets.get_at_index(index).insert(cur.key, cur.value)
                    newHM.size += 1
